...
function __OriginkitBase_ParticleText(rawProps: Props) {
    const props = { ...COMPONENT_DEFAULTS, ...rawProps } as Props
    const {
        text,
        colors,
        mode,
    ...
    useEffect(() => {
        ...
        const sampleText = () => {
            ...
            const maxW = W * 0.92
            const maxH = H * 0.92
            let effectiveSize = Math.max(8, fontSize ?? 80)
            if (autoFit) {
            ...
            const pCount = Math.max(1, Math.min(50, particleCount ?? 50))
        ...
        const drawFrame = () => {
            ...
            const drawSize = Math.max(1, (particleSize ?? 12) / 4)
...
export default function ParticleText(props: Record<string, unknown>) {
  return <__OriginkitBase_ParticleText {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
